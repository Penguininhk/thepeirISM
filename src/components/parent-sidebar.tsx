      ...
      <SidebarHeader>
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-xs text-sidebar-foreground">Parent Portal</p>
          </div>
        </div>
          ...
          {menuItems.map((item) => (
            <SidebarMenuItem key={item.label}>
              <Link href={item.href} passHref>
                <SidebarMenuButton
                  isActive={
                    (item.href === '/dashboard/parent' && pathname === item.href) ||
                    (item.href !== '/dashboard/parent' && pathname.startsWith(item.href))
                  }
                  tooltip={item.label}
                >
                  <item.icon />
